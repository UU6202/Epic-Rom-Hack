...
;Expanded by NovaSquirrel

.include "defines.asm"
.include "text.asm" ; use SMB1's font instead of ASCII
.macpack generic    ; defines add/sub
.macpack longbranch ; defines jeq/jne and so on, that act like beq/bne but with a bigger range

.segment "INESHDR"
  .byt $4E, $45, $53, $1A ; "NES"
  .byt 8 ; PRG in 16kb units
  .byt 1 ; CHR in 8kb units
  .byt (4<<4) & $f0 ; use the MMC3 mapper (number 4)
  .byt (4 & $f0)    ; (it's split over two bytes)
.segment "VECTORS"
; having a segment for just the vectors means they will stay where they need to be
; even if the rest of the code is moved around
  .word NonMaskableInterrupt
  .word InitMapper
  .word $fff0  ;unused
.segment "CHR"
  .incbin "Graphics/CHR (1).chr"
  .incbin "Graphics/CHR (2).chr"
  .incbin "Graphics/CHR (3).chr"
  .incbin "Graphics/CHR (4).chr"
  .incbin "Graphics/CHR (5).chr"
  .incbin "Graphics/CHR (6).chr"
  .incbin "Graphics/CHR (7).chr"
  .incbin "Graphics/CHR (8).chr"
.segment "PRG0"
  .include "sound.asm"
.segment "PRG1"
  .include "levels.asm"
...
WorldLivesDisplay:
  ...
  .byt $21, $4b, $09, "WORLD  - " ; "WORLD  - " used on lives display
...
TwoPlayerTimeUp:
  .byt $21, $cd, $05, "MARIO"
OnePlayerTimeUp:
  .byt $22, $0c, $07, "TIME UP"
  .byt $ff

TwoPlayerGameOver:
  .byt $21, $cd, $05, "MARIO"
OnePlayerGameOver:
  .byt $22, $0b, $09, "GAME OVER"
  .byt $ff

WarpZoneWelcome:
  .byt $25, $84, $15, "WELCOME TO WARP ZONE!"
...
LuigiName:
  .byt "LUIGI"   	 ;  no address or length
...
ProcLoopb:  dex                      ;decrement buffer offset
            ...
            jne ProcessAreaData      ;branch if true to load more level data, otherwise
            lda BackloadingFlag      ;check for flag set if starting right of page $00
            jne ProcessAreaData      ;branch if true to load more level data, otherwise leave
EndAParse:  rts
...
; therefore, the code for the reset vector needs to be located in there.
; This code sets up the MMC3 and then jumps to SMB1's init code.
.segment "INIT"
MapperInitValues:
             .byte 0 ; CHR
             .byte 2
...
:            stx MMC3_BANK_SELECT ; select a register
             lda MapperInitValues,x
             sta MMC3_BANK_VALUE ; write the value from the table
             inx
             cpx #$08
             bne :-

             sta MMC3_DISABLE_IRQ ; Disable MMC3 interrupts

             jmp Start ; continue with the game
.segment "CODE"
